...
export function EmailCaptureCard({ onSubmit, sessionId }: EmailCaptureCardProps) {
  ...
  // Load split test variant
  useEffect(() => {
    getComponentVariant('component:email_capture').then(v => {
      setVariant(v)
      if (v && !hasTrackedImpression.current) {
        trackComponentImpression(v.testName, v.variant, 'email_capture', sessionId)
        hasTrackedImpression.current = true
      }
    })
  }, [sessionId])

  // Autofocus the input after the entrance animation completes
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (variant) {
        trackComponentConversion(variant.testName, variant.variant, 'email_capture', sessionId)
      }
      setIsSuccess(true)
    } catch {
  ...
  // Success state - compact inline confirmation
  if (isSuccess) {
    return (
      <div className="self-start w-full max-w-lg animate-in fade-in duration-300">
        <div className="flex items-center gap-3 px-6 py-4 bg-gradient-to-br from-teal-50/60 to-white rounded-2xl border border-teal-100/80">
          ...
          <span className="text-base font-medium text-teal-800">
            Saved! We'll send your report when we're done.
          </span>
        </div>
      </div>
    )
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h4 className="text-base font-bold text-gray-900">
                {variant?.headline || "Don't let these insights slip away..."}
              </h4>
              <p className="text-sm text-gray-500 mt-1 leading-relaxed">
                {variant?.description || "Save your progress now so you don't lose any key insights. And once we're done, you'll get a personalized report with your key breakthroughs and next steps."}
              </p>
            </div>
          </div>
        </div>
